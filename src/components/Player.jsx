...
const Player = () => {
    ...
    if (state.action === "play" && state.audio) {
      interval = setInterval(() => {
        setPlayerTime((prevTime) => {
          ...
          if (newTime >= state.audio.time) {
            clearInterval(interval);
          }
          return newTime < state.audio.time ? newTime : state.audio.time;
        });
      }, 1000);
    } else {
      clearInterval(interval);
  ...
  const nextHandler = () => {
    const newIndex = count >= state.musics.length - 1 ? 0 : count + 1;
    setCount(newIndex);
    setAudio(newIndex);
  };
  const prevHandler = () => {
    const newIndex = count < 1 ? state.musics.length - 1 : count - 1;
    setCount(newIndex);
    setAudio(newIndex);
  };
